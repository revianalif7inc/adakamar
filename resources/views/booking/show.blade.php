                    ...
                    <div class="col-md-4">
                        <div class="thumb mb-3">
                            ...
                            @if($img && \Illuminate\Support\Facades\Storage::disk('public')->exists($img))
                                <img src="{{ asset('storage/' . $img) }}" alt="{{ $booking->homestay->name }}" class="img-fluid rounded">
                            @else
                                <img src="{{ asset('images/homestays/placeholder.svg') }}" alt="placeholder" class="img-fluid rounded">
                            @endif
                        </div>
                    ...
                    <div class="col-md-8">
                        <div class="mb-3">
                            ...
                            <table class="table table-borderless">
                                <tr><td>Subtotal</td><td class="text-end">Rp {{ number_format($booking->total_price, 0, ',', '.') }}</td></tr>
                                <tr><td>Diskon</td><td class="text-end">Rp 0</td></tr>
                                <tr class="fw-bold"><td>Total</td><td class="text-end">Rp {{ number_format($booking->total_price, 0, ',', '.') }}</td></tr>
                            </table>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="booking-timeline mb-3">
                                ...
                                <div class="d-flex gap-2 flex-wrap">
                                    @foreach($steps as $key => $label)
                                        <div class="timeline-step {{ $booking->status === $key ? 'active' : (array_search($key, array_keys($steps)) < array_search($booking->status, array_keys($steps)) ? 'done' : '') }}">
                            ...
                            <div>
                                @if(auth()->id() === $booking->user_id && $booking->status === 'pending')
                                    <a href="{{ route('booking.pay.form', $booking->id) }}" class="btn btn-primary me-2">Bayar / Unggah Bukti</a>
                                @endif
                                ...
                                @if($isOwner || $isAdmin)
                                    <form action="{{ $isAdmin ? route('admin.bookings.updateStatus', $booking->id) : route('owner.bookings.updateStatus', $booking->id) }}" method="POST" class="d-inline-block mt-2">
                                        @csrf
                                        @method('PUT')
                                ...
                                {{-- Admin: delete action --}}
                                @if($isAdmin)
                                    <form action="{{ route('admin.bookings.destroy', $booking->id) }}" method="POST" class="d-inline-block ms-2">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <div class="mb-3">
                            ...
                            @if($booking->homestay->owner)
                                <p>{{ $booking->homestay->owner->name }} — {{ $booking->homestay->owner->phone ?? $booking->homestay->owner->email }}</p>
                            @endif
                        </div>
